...
def build_model():
    ...
    x = Dense(dense_dim, activation='relu', name='finetunepssmDense_1')(merge_vector)
    x = keras.layers.LeakyReLU(alpha=0.3)(x)
    x = Dense(int((dense_dim)/2), activation='relu', name='finetunepssmDense_2')(x)
    x = keras.layers.LeakyReLU(alpha=0.3)(x)
    main_output = Dense(2, activation='softmax', name='finetunepssmDense_3')(x)
...
for train, test in train_test:
    ...
    # Save transfer learning model (fine-tuning type)
    merge_model.save('../results/'+target_virus+'_'+source_virus+'_transfer_finetune'+str(n_model)+'.h5')
    # Test the model
    ...
    w = open('../results/'+target_virus + '_' + source_virus + '_transfer_finetune_score' + str(n_model), 'w')
